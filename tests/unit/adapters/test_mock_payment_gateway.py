...
class TestMockPaymentGateway:
    ...
    def test_get_subscription_status(self) -> None:
        create_result = self.gateway.create_subscription("user_123", 9.95, "pm_test")
    ...
    def test_webhook_handler_registration(self) -> None:
        ...
        assert len(calls) == 1
        assert calls[0][0] == "subscription.canceled"
        assert calls[0][1]["subscription_id"] == create_result.subscription_id

    def test_reset(self) -> None:
        self.gateway.create_subscription("user_123", 9.95, "pm_test")
        self.gateway.reset()
