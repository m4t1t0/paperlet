...
class NotificationService:
    ...
    def send_post_published_notifications(
        ...
        if allocation_recipients:
            ...
            self._sender.send_batch(allocation_recipients, "post_published_full", {
                "subject": subject,
                "html": html,
                "post_title": post.title,
                "post_full": post.subscriber_content,
        ...
        if preview_recipients:
            ...
            self._sender.send_batch(preview_recipients, "post_published_preview", {
                "subject": subject,
                "html": html,
                "post_title": post.title,
                "post_preview": post.preview_content,
...
def get_subscribers_with_allocation(post_writer_id: UUID) -> list[tuple["User", "Subscription"]]:
    """Get all users with active allocation to a writer."""
    # This would be implemented with a read model query
    # For now, return empty list - will be implemented in Celery task
    return []
